...
class pegawai :
    ...
    def insert_data(val1,val2,val3,val4,val5,val6):
        sql = "INSERT INTO pegawai (nip, nama, hp, jabatan, alamat, departemen) VALUES (%s, %s, %s, %s, %s, %s)"
    ...
    def update_data(val1,val2,val3,val4,val5,val6):
        sql = "UPDATE pegawai SET nama = %s, hp = %s, jabatan = %s, alamat = %s, departemen = %s WHERE nip = %s"
        val = (val1,val2,val3,val4,val5,val6)
        mycursor.execute(sql, val)
        mydb.commit()
